/* 
As regras para o cálculo dos anos bissextos são as seguintes:
De 4 em 4 anos é ano bissexto;
De 100 em 100 anos não é ano bissexto;
De 400 em 400 anos é ano bissexto;
Prevalecem as últimas regras sobre as primeiras.
Partindo daí elabore uma função que recebe um ano e calcula se ele é ano bissexto,
imprimindo no console a mensagem e retornando true ou false.
*/

function anoBissexto (ano) {

    let anoComDoisZeros = ano % 100 === 0
    let anoBissextoSemZeros = ano % 4 === 0
    let anoBissextoComZeros = ano % 400 === 0

    if (anoComDoisZeros) {
        if (anoBissextoComZeros) {
            return anoBissextoComZeros
        } else {
            return anoBissextoComZeros
        }
    } else if (anoBissextoSemZeros) {
        return anoBissextoSemZeros
    } else {
        return anoBissextoSemZeros
    } 
}

console.log(anoBissexto(2016));
console.log(anoBissexto(1900));
console.log(anoBissexto(2100));
console.log(anoBissexto(2020));
console.log(anoBissexto(2024));
